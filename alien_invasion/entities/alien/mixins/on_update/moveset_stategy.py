...
def on_update_plot_movement(
    ...
    def get_alien_count_proportion_on_x_axis() -> float:
        ...
        aliens_count_on_right = len(tuple(filter(
            lambda a: a.center_x > alien.center_x,
            alien._parent_sprite_list
        ))) or 1
    ...
    if (
        (
            AlienMoveset.persuing in movesets or alien._overrides.should_persue
        ) and (
    ...
    if AlienMoveset.tracking in movesets:
        if not alien.dodging:
            if math.fabs(alien.center_x - ship_x) < starship.width / starship.width * random():
                alien.change_x = 0
    ...
    if alien.center_y < starship.top * 1.33:
        alien.change_x = 0
